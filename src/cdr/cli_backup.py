  ...
  atc-llm metrics --results-dir results/latest/
    ...
    def _run_e2e(self, args) -> int:
        ...
        try:
            ...
            # Pass the adapter object (not raw data) to the pipeline
            # The pipeline will call adapter.to_simple_scenario() to get the right format
            scenario_data = scat_adapter
            ...
            # Initialize CDR pipeline with more sensitive conflict detection
            pipeline_config = PipelineConfig(
                cycle_interval_seconds=60.0,
                lookahead_minutes=10.0,
                max_simulation_time_minutes=480.0,  # 8 hours for long-distance flights
                separation_min_nm=2.0,  # More sensitive - reduced from 5.0 to 2.0 NM
                separation_min_ft=500.0,  # More sensitive - reduced from 1000 to 500 ft
                detection_range_nm=50.0,  # Reduced range for better performance
                llm_enabled=True,
                memory_enabled=True,
            ...
            simulation_result = cdr_pipeline.run_scenario(scenario_data, output_dir)
            ...
            for key, value in vars(args).items():
                ...
                if callable(value) or key == 'func':
                    continue
                elif hasattr(value, '__fspath__'):  # It's a Path object
